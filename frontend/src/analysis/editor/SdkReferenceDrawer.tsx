...
export function SdkReferenceDrawer() {
  return <details className="analysis-panel">
    ...
    <p><strong>Mapping points:</strong> <code>{'event.values().map((value, point) => point.withValue((value ?? 0) * 2))'}</code></p>
    <p><strong>Mapping values:</strong> <code>{'event.values().mapValues((value) => value * 2)'}</code></p>
    <p><strong>Typed UDF factories:</strong> <code>{'event.values().map(udf.mappers.clamp(0, 100))'}</code>. Configuration belongs in typed parameters, not an options bag.</p>
