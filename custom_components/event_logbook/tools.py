...
def _result(action: str, event: dict[str, Any]) -> JsonObjectType:
    return {
        "success": True,
        "action": action,
        "event": event,
        "instruction": "Relay this result to the user using local human-readable times and omit absent fields.",
    }
...
class ListEventTypesTool(BaseLogbookTool):
    ...
    async def async_call(self, hass: HomeAssistant, tool_input: ToolInput, llm_context: LLMContext) -> JsonObjectType:
        del hass, tool_input, llm_context
        return {
            "success": True,
            "eventTypes": [
                {
...
class LogPointEventTool(BaseLogbookTool):
    ...
    async def async_call(self, hass: HomeAssistant, tool_input: ToolInput, llm_context: LLMContext) -> JsonObjectType:
        ...
        payload = {"eventTypeKey": event_type.key}
        payload.update(_optional_payload(args, {
            "occurred_at": "occurredAt", "value": "value", "unit_key": "unitKey",
            "text_value": "textValue", "note": "note",
        }))
        try:
            return _result("logged_point_event", await self.client.async_log_point(payload, tool_input.id))
...
class StartDurationEventTool(BaseLogbookTool):
    ...
    async def async_call(self, hass: HomeAssistant, tool_input: ToolInput, llm_context: LLMContext) -> JsonObjectType:
        ...
        payload = {"eventTypeKey": event_type.key}
        payload.update(_optional_payload(args, {
            "started_at": "startedAt", "value": "value", "unit_key": "unitKey",
            "text_value": "textValue", "note": "note",
        }))
        try:
            return _result("started_duration_event", await self.client.async_start_duration(payload, tool_input.id))
        except LogbookApiError as err:
            _raise_tool_error(err)
...
class FinishDurationEventTool(BaseLogbookTool):
    ...
    async def async_call(self, hass: HomeAssistant, tool_input: ToolInput, llm_context: LLMContext) -> JsonObjectType:
        ...
        payload = {"eventTypeKey": event_type.key}
        payload.update(_optional_payload(args, {
            "ended_at": "endedAt", "value": "value", "unit_key": "unitKey",
        }))
        try:
            return _result("finished_duration_event", await self.client.async_finish_duration(payload, tool_input.id))
        except LogbookApiError as err:
            _raise_tool_error(err)
...
class GetLatestEventTool(BaseLogbookTool):
    ...
    async def async_call(self, hass: HomeAssistant, tool_input: ToolInput, llm_context: LLMContext) -> JsonObjectType:
        ...
        try:
            return _result("retrieved_latest_event", await self.client.async_get_latest(event_type.key))
        except LogbookApiError as err:
            _raise_tool_error(err)
